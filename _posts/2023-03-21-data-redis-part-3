...
subtitle:  Data & Redis - Data Processing with RDI (Hands-on)
thumbnail-img: assets/img/RDI-2.svg
share-img: assets/img/RDI-2.svg
tags: [Debezium,Hands-On,RDI,stream processing,data processing,data transformation,redis]
comments: true
---

Data processing is in the core of any data architecture. It involves transforming raw data into useful insights through analysis techniques like machine learning algorithms or statistical models depending on what type of problem needs solving within an organization's context. 

We have seen in the past posts that raw data already extracted from data sources, can be prepared and transformed (using Redis Gears) into the target format required by the downstream systems. In this post, we push this concept further by coupling the event-processing of RedisGears and the stream-based ingestion using Redis Data Integration (RDI). Thus, you can imagine that data flowing in your operational systems (eg., ERPs, CRMs...) will be ingested into Redis using a Change Data Capture (see [Data & Redis - part 1](https://aelkouhen.github.io/2023-02-21-data-redis-part-1/)) and processed with RedisGears to derive rapid operational decisions in near real-time. 

In fact, Redis Data Integration is not only a data integration tool, but also a data processing engine that relies on Redis Gears. It provides simpler way to implement data transformations (declarative files) to avoid the complexity of Redis Gears.

## Pre-requisites

### 1 - Create a Redis Database 

For the purpose of this article, you need to install and set-up a few things. First of all, you need to prepare a Redis Enterprise Cluster which is the sink storage support. This storage support will be the target infrastructure for the data transformed in this stage. You can use this [project](https://github.com/amineelkouhen/terramine) to create a Redis Enterprise cluster in the cloud provider of your choice.

Once you have created a Redis Enterprise cluster, you have to create a target database that holds the transformed data. Redis Enterprise Software lets you create databases and distribute them across a cluster of nodes. To create a new database, follow the instructions [here](https://docs.redis.com/latest/rs/databases/create/). For Redis Data Integration, you need two databases : the config database exposed on `redis-12000.cluster.redis-process.demo.redislabs.com:12000` and the target database on : `redis-13000.cluster.redis-process.demo.redislabs.com:13000`. Don't forget to add the RedisJSON module when you create the target database.
...
Redis Data Integration (RDI) leverages the hybrid approach since all transformation jobs are implemented using these human readable format (yaml files) that embeds JMESPath and/or SQL.

![](https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEg1RIyS7F9cX57GvSwT6djnZPyVA1vKr9wjNPZJiI_zB7M9ufEN308mV6RtIieIrtgQXnH5OuWTKY5SDwm4gOhzq4lKHX6b-bD46cgKbU30-4SQD6yVVEZwkaPMM-k2qD7wWqbZ4wY0_I6uIEoLWB8Gt0MotycqKDapLH_pv36Kt9GLYtK_x6pMyrgI){: .mx-auto.d-block :} 
...
I've detailed in [Data 101 - part 5](https://aelkouhen.github.io/2023-02-08-data-101-part-5/) many data transformations archetypes and I find it interesting to evaluate Redis Data Integration through this list of capabilities. Thus, you can see how to perform different kinds of transformations using RDI.

### 1- Filtering

This process selects a subset from your dataset (certain columns) that require transformation, viewing or analysis. This selection can be based on certain criteria, such as specific values in one or more columns, and it typically results in only part of the original data being used. Filtering allows you to quickly identify trends and patterns within your dataset that may not have been visible before. It also enables you to focus on particular aspects of interest without having to sift through all available information. In addition, this technique can help reduce complexity by eliminating unnecessary details while still preserving important insights about the underlying data structure.
...
Let's assume the [example](https://raw.githubusercontent.com/aelkouhen/aelkouhen.github.io/main/assets/data/mssql_script_emp_enrich) above. We need to replace all NULL salaries in the Employee's table by a default value 0. In SQL the `COALESCE` function returns the first non-NULL value in the list of attribute. Thus `COALESCE(SAL, 0)` returns the salary if it is not null or 0 elsewhere. With RDI we can implement this enrichment using the following job :
...
Splitting a fields into multiple ones consists of two atomic operations : adding the new fields according to specific transformation rules, then removing the source field (split column).
...
In the [example](https://raw.githubusercontent.com/aelkouhen/aelkouhen.github.io/main/assets/data/mssql_script_emp_split) above, we split the `EFULLNAME` into two fields : `ELASTNAME` and `EFIRSTNAME`. The following configuration, uses the add\_field block to create the new fields `ELASTNAME` and `EFIRSTNAME`. For this, we can use the `SUBSTRING` function from SQL or the `SPLIT` function from JMESPath. In both case, we need the additional block `remove_field` to remove the source column `EFULLNAME`.
...
Besides removing specific columns from the source using the `remove_field` block or even avoiding the load of some columns using filtering. We might need to drop parts of data according to a specific condition such as duplicates for example. In this case, Redis Data Integration doesn't have a specific block or function to perform the drop of duplicates. However, we can use the key block to create a custom key for the output that is composed of all fields that form the duplicate.
...
In addition, we use the hash function to create an ID instead of a set of concatenated fields. Beware that It may be possible that two concatenations (different strings) have the same hash values. This may occur because we take modulo 'M' in the final hash value. In that case, two different combination of (EFULLNAME '-' SAL) may have the same hash values, called a collision. 

However, for a set of even billions of assets, the chances of random collisions are negligibly small. Because the SHA-3 series is build to offer 2^n/2 collision resistance. In our transformation we've chosen SHA3-512 which offers 2^256 (or 1 over 115,792,089,237,316,195,423,570,985,008,687,907,853,269,984,665,640,564,039,457,584,007,913,129,639,936 possibilities to get another String combination having the same hash !
...
Derivation: is cross column calculations. With RDI, we can easily create a new field based on calculations from existing fields. Let's assume the [example](https://raw.githubusercontent.com/aelkouhen/aelkouhen.github.io/main/assets/data/mssql_script_emp_derivate) blow, we need to calculate the total compensation of each employee based on their salaries and bonuses they get.
...
However, you can chose to perform the denormalization using JSON format, in this case no duplication will be represented, thus no impact on the storage since the parent-children relationship is just reflected in a more hierarchical way.

Let's assume the [two tables](https://raw.githubusercontent.com/aelkouhen/aelkouhen.github.io/main/assets/data/mssql_script_emp_dept_denorm): Department, and Employee. we will create a declarative data transformation that denormalize these two tables into one nested structure in JSON. The aim is to get the details of employees in each department.
...
Run this T-SQL script for each table in the database and substitute the table name in `@source_name` with the tables names (Employee and Department) :
...
In the RDI configuration file config.yaml, you need to add some the following settings.
...
{: .box-warning}
**Caution:** If you want to execute normalization/denormalization jobs, you need first to get the build 0.95 of Redis Data Integration, since it still under development.  

#### For UBUNTU18.04

```shell
wget https://qa-onprem.s3.amazonaws.com/redis-di/0.95.0b3/redis-di-ubuntu18.04-0.95.0b3.tar.gz -O ~/tmp/redis-di-offline.tar.gz
```

Then you install the RDI CLI by unpacking `redis-di-offline.tar.gz` into `/usr/local/bin/` directory:

```shell
sudo tar xvf ~/tmp/redis-di-offline.tar.gz -C /usr/local/bin/
```

Upgrade for your Redis Data Integration (RDI) engine to be compliant with the new `redis-di` CLI. For this execute :  
...
Then, run deploy command to deploy the local configuration to the remote RDI config database:
...
Redis Data Integration (RDI) performs data denormalization in a performant and complete manner. It is not only structuring the source tables into one single structure, but it can handle in the same way the late arriving data : If the nested data is captured before the parent-level data, RDI creates a JSON structure for the child-level records and as soon as the parent-level data arrives, RDI creates the structure for the parent-level record, then merges all children (nested) records into their parent structure. Let's consider these two tables : [Invoice](https://raw.githubusercontent.com/aelkouhen/aelkouhen.github.io/main/assets/data/mssql_script_invoice_denorm) and [InvoiceLine](https://raw.githubusercontent.com/aelkouhen/aelkouhen.github.io/main/assets/data/mssql_script_invoice_line_denorm). When you try to insert an InvoiceLine contained by an Invoice before this later, RDI will create the JSON structure for InvoiceLine and wait for the Invoice structure. As soon as you insert the containing Invoice, RDI creates the Invoice JSON structure and merge it with the InvoiceLines created earlier.
...
One of the issues observed so far with RDI's denormalization is the nesting limit (limited to one level). It is not possible for the moment to denormalize more than two tables having one-to-many or many-to-one relationships.

### 8- Data Normalization

In addition to data ingest, Redis Data Integration (RDI) allows also to synchronize the data stored in a Redis DB with some downstream data store. This scenario is called "Write-Behind" and you can think about it as a pipeline that starts with Capture Data Change (CDC) events for a Redis DB and then filters, transforms and maps the data to a target data store (eg., a relational database). 

![](https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjxEAis-3v8Zm_TpE2BFDpljz5azHan10JwfqepJjIhTSiF_X8JUhWp-fm-boGbiRWGYXxhJCNW2HZYP6mJoPevdyKuSNbQKb0dr_JI15W8xjMBkRbKo5jgRXfLtawedIsVzcnTr3JR1zrLRm8TRlO0q9VbYcPTCsOis8s3rv0dTuCkszdkU7rVP7Dy){: .mx-auto.d-block :} *Redis Data Integration use cases*{:style="display:block; margin-left:auto; margin-right:auto; text-align: center"} 
  
We've seen in the last section that we can perform data denormalization, to join multiple table that have one-to-many or many-to-one relationships, into one single structure in Redis. In the other side, one of the transformations we can perform using the Write-Behind use-case is the data normalization. Data normalization is the process of organizing data into a logical structure that can be used to improve performance and reduce redundancy. This involves breaking down complex datasets into smaller, more manageable pieces by eliminating redundant information or consolidating related items together. Normalization also helps ensure consistency in how data is stored and accessed across different systems.
...
Let's assume this [JSON document](https://raw.githubusercontent.com/aelkouhen/aelkouhen.github.io/main/assets/data/invoice.json), stored in Redis. It consists of an Invoice with the details it contains (InvoiceLines), and we want to normalize this structure into two distinct tables :  a table that contains invoices and another table containing invoice lines. For example, with a single nested structure (one invoice composed of three invoice lines), we should have in the target two tables containing four records : one record in the invoice table and three records in the invoice line table. 

In this section, we will use this database `redis-13000.cluster.redis-process.demo.redislabs.com:13000` as data source. This database must include RedisGears and RedisJSON modules to execute the following actions.

First of all, you need to create install the RDI engine on your Redis source database so it is ready to process data. For this, you need to run the [`configure`](https://redis-data-integration.docs.dev.redislabs.com/reference/cli/redis-di-configure.html) command if you have not used this Redis database with RDI Write Behind before.
...
Let's assume that your target mySQL database endpoint is `rdi-wb-db.cluster-cpqlgenz3kvv.eu-west-3.rds.amazonaws.com`, you need to add the connection(s) required for downstream targets in the `connections` section of `config.yaml`, for example :
...
Now, let's create the following file in the jobs directory. This declarative file splits the JSON structure and creates the two tables in a mySQL database called sales. You are able to define different targets for these two tables by defining different connections in the config.yaml file.
...
In this article, we've seen how to perform complex data transformations using Redis Data Integration (RDI). This is my second post on RDI, since I presented it in the past as a data ingestion tool. Here we pushed the data journey further and used RDI as a data processing and transformation engine. 

In the previous sections, I presented a set of data transformation scenarios that are more often required in any enterprise-grade data platforms and tried to assess RDI capabilities accordingly. The tool still under heavy development and private previews, but it offers many promising capabilities to implement a complete real-time data platform.
