...
In the early years of computing, "consistency" had a clear home: the database. If your transactions were atomic and your indexes balanced, you could sleep at night knowing your system was sound. But the world changed. Data escaped the monolith, spreading into microservices, caches, event buses, analytics pipelines, and authorization systems. Today, an end-user action can ripple through half a dozen systems: an API gateway, an identity provider, an authorization service, and multiple replicated databases, all in different regions. Somewhere in that chain, each system needs to agree not only on what happened, but also who is allowed to know about it. That's the new shape of consistency: not just data correctness, but permission correctness. It's about ensuring that information and permissions evolve together in a distributed world.

This article explores how [distributed SQL](https://www.cockroachlabs.com/glossary/distributed-db/distributed-sql/) and distributed authorization systems face the same core challenges — and how pairing [CockroachDB](https://www.cockroachlabs.com/product/overview/) with [AuthZed](https://authzed.com)'s [SpiceDB](https://authzed.com/spicedb) creates a globally consistent foundation for data and policy.
...
1. **Data Consistency:** The database's view of the world.
2. **Permissions Consistency:** The authorization system's understanding of who can access what.
3. **Operational Consistency:** The coherence of event streams, caches, and background jobs that rely on that data.
...
The goal isn't perfect simultaneity — that's physically impossible in distributed systems — but predictable coherence: every component observing a consistent timeline of events.

Just as databases evolved to handle distributed consistency, authorization systems are undergoing a similar transformation — and that's where CockroachDB's guarantees matter.
...
On the SpiceDB side, install the binary/container or use the Kubernetes Operator, then run the following command:
...
To interact with SpiceDB through the `zed` (AuthZed) CLI, install the latest binary releases using the official tap:

```bash
brew install authzed/tap/zed
```

Once installed, connect to the SpiceDB instance with the command below. For local development, use the `--insecure` flag to connect over plaintext. Be sure to use the same `preshared_key` used in the `spicedb serve` command:
...
If the output of `zed version` shows the server version as "unknown," then your CLI was unable to connect — double check the `preshared_key`, the IP, or the port your SpiceDB instance is running on.

### Defining the Schema

Writing one or more object type definitions is the first step in developing an authorization relationship schema.

In the example below, we define the `user` and `document` concepts. The user can be a `viewer`, an `editor`, or an `admin`. The definition gives the `remove` permission to the `admin` role only. To `edit` a file the user must be either an `editor` or `admin`. The permission to `view` a document is set for viewer, editor and admin roles:
...
First, save the schema created earlier as `schema.zed` and execute:

```bash
zed schema write ./schema.zed
```

If it works, the authorization schema will be printed when running:

```bash
zed schema read
```

There is another less recommended way to store the schema in SpiceDB using the REST API, where the schema definition is embedded into the request body:
...
Let's simulate a new content creation flow: a user `amine` creates a new document called `doc1` and becomes its `admin`, meaning he can do everything to `doc1` (view, edit, and remove). Let's also add user `jake` with the viewer role for `doc1`:
...
Now `jake` can only view `doc1`. We can seed another test user `evan` and add him as `editor` of `doc1` using the REST API:
...
To verify our schema is working correctly, issue a few check requests. As `jake` is only a `viewer` for `doc1`, we expect him to have the view permission but not the `edit` or `remove` permissions:
...
Let's also check that `jake` doesn't have the `remove` permission on `doc1`, this time using the REST API:
...
*[Let's talk about reliable permissions.](https://www.cockroachlabs.com/contact/)*
