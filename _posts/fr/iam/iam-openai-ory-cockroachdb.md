...
Au cœur de ce schéma se trouve la table `hydra_client`, qui définit les clients OAuth2/OIDC enregistrés au sein d'un Réseau Hydra. Ces clients représentent des applications qui peuvent initier des flux d'autorisation et demander des tokens d'accès. Chaque flux majeur, notamment le code d'autorisation, le token d'accès, le token de rafraîchissement et l'autorisation de dispositif, est lié à un client spécifique.
